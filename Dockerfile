...
# Version files deliberately participate in Docker's cache key.
COPY XUI_VERSION /tmp/XUI_VERSION
COPY CLOUDFLARED_VERSION /tmp/CLOUDFLARED_VERSION


# Install 3x-ui during IMAGE BUILD.
RUN set -eux; \
    version="$(tr -d '\r\n ' < /tmp/XUI_VERSION)"; \
    ...
    if [ -d /app/x-ui/bin ]; then \
        find /app/x-ui/bin -type f -exec chmod 0755 {} \;; \
    fi; \
    rm -f /tmp/xui.tar.gz
...
# 3x-ui writable data directory.
RUN rm -rf /app/data \
    && mkdir -p /tmp/xui-data \
